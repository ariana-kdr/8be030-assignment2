...
def t2h(T, t):
    ...
    TN = np.concatenate((T, N))
    tn = np.append(t, 1)
    tn = np.reshape(tn, (-1, 1))

    Th = np.concatenate((TN,tn), axis = 1)

    # you could make this neater using 
    # Th = np.block([[T                        , np.array(t).reshape(-1, 1)],
    #                [np.zeros((1, T.shape[1])), 1                         ]])	
    #------------------------------------------------------------------#
