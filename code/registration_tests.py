...
def combining_transforms():
    ...
    X_affine1 = reg.reflect(-1,1).dot(reg.rotate(np.pi/2)).dot(X)
    X_affine2 = reg.rotate(np.pi/2).dot(reg.reflect(-1,1)).dot(X)
    
    fig = plt.figure(figsize=(5,5))
    ax  = fig.add_subplot(111)  
    ax.grid()
    util.plot_object(ax, X)
    util.plot_object(ax, X_affine1)
    util.plot_object(ax, X_affine2)
    #------------------------------------------------------------------#
...
def arbitrary_rotation():
    ...
    T1 = util.t2h(reg.identity(), -Xt) # only translation
    R = util.t2h(T_rot, np.zeros(2))   # only rotation
    T3 = util.t2h(reg.identity(), Xt)  # only translation
    T = T3.dot(R).dot(T1) 
    #------------------------------------------------------------------#
...
def correlation_test():
    ...
    I = plt.imread('../data/cameraman.tif')
    Th2 = util.t2h(reg.identity(), np.array([10,20]))
    J2, _ = reg.image_transform(I, Th2)
    ...
    if abs(C1 - 1) < 10e-10:
        print('The correlation of I with itself is ', C1, ', Test successful!')
            
    C2 = reg.correlation(I, J2)
    print('The correlation of I with J2 (translation 10,20) is ', C2)

    Th3 = util.t2h(reg.reflect(-1,1), np.array([0,0]))
    J3, _ = reg.image_transform(I, Th3)
    C3 = reg.correlation(I, J3)
    print('The correlation of I with J3 (reflection -1,1) is ', C3)
...
def mutual_information_test():
    ...
    # MI1, H1 = reg.mutual_information(p1)            # added H as output

    #------------------------------------------------------------------#
    # Implement a few tests of the mutual_information definition
    # commented to reduce runtime
    # noise1 = np.random.randint(255, size=(512, 512))
    # noise2 = np.random.randint(255, size=(512, 512))
    # p2 = reg.joint_histogram(noise1, noise2) 
    # MI2, H2 = reg.mutual_information(p2) 
    # print('The mutual information when I and J are random noise is ', MI2, ' and the Shannon entropy is ', H2)
    #------------------------------------------------------------------#

    # if round(MI1, 3) == round(H1, 3):               # check they're roughly equal (added the condition) 
    #     print('Test successful! The mutual information when I=J is ', MI1, ' and the Shannon entropy is ', H1)
...
def mutual_information_e_test():
    ...
    # mutual information of an image with itself
    p1 = reg.joint_histogram(I, I)
    MI_e = reg.mutual_information_e(p1)
    # MI, H1 = reg.mutual_information(p1)
    MI = reg.mutual_information(p1)
    assert abs(MI_e-MI) < 10e-3, "Mutual information function with entropy is incorrectly implemented (difference with reference implementation test)"

    print('Test successful!', MI_e, MI)
...
def ngradient_test():
    ...
    if abs(g1 - exponential(1)) < 1e-5:               # added myself 
        print('Test successful!')
