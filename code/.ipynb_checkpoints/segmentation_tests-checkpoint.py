...
def scatter_data_test(showFigs=True):
    # read first image - original
    I = plt.imread('../data/dataset_brains/1_1_t1.tif')
    # create first X rows with feature m1 - [height*width, 1] 
    X1 = I.flatten().T
    X1 = X1.reshape(-1, 1)
    # read ground truth (segmented) image 
    GT = plt.imread('../data/dataset_brains/1_1_gt.tif')
    # create labels Y with true/false - [height*width, 1] 
    gt_mask = GT>0
    Y = gt_mask.flatten() # labels

    # read second image - blurred with sigma = 2
    I2 = ndimage.gaussian_filter(I, sigma=2)
    # create second X rows with feature m2 
    X2 = I2.flatten().T
    X2 = X2.reshape(-1, 1)
    ...
    # read third image - blurred with sigma = 25
    I3 = ndimage.gaussian_filter(I, sigma=25)
    X3 = I3.flatten().T
    X3 = X3.reshape(-1,1)

    # read fourth image - blurred with sigma = 50
    I4 = ndimage.gaussian_filter(I, sigma=50)
    X4 = I4.flatten().T
    X4 = X4.reshape(-1,1)

    # stack the columns to create a 1D array 
    X_data = np.concatenate((X1, X2, X3, X4), axis=1)

    # Keep track of features you added
    features = ('T1', 'T1 σ=2', 'T1 σ=25', 'T1 σ=50'  )

    if showFigs:
        # plot original image against blurred with σ=2
        ax1 = util.scatter_data(X_data,Y,0,1)
        ax1.legend()
        ax1.set_title(f'{features[0]} vs {features[1]}')

        # plot original image against blurred with σ=10
        ax2 = util.scatter_data(X_data,Y,0,2)
        ax2.legend()
        ax2.set_title(f'{features[0]} vs {features[2]}')

        # plot original image against blurred with σ=50
        ax3 = util.scatter_data(X_data,Y,0,3)
        ax3.legend()
        ax3.set_title(f'{features[0]} vs {features[3]}')

        # plot blurred with σ=25 against blurred with σ=50
        ax4 = util.scatter_data(X_data,Y,2,3)
        ax4.legend()
        ax4.set_title(f'{features[2]} vs {features[3]}')        
...
def scatter_t2_test(showFigs=True):
    ...
    X1 = I1.flatten().T.reshape(-1, 1)                          # T1 raw
    X1_s2 = ndimage.gaussian_filter(I1, sigma=2).flatten().T.reshape(-1, 1)   # T1 σ=2
    X1_s25 = ndimage.gaussian_filter(I1, sigma=25).flatten().T.reshape(-1, 1) # T1 σ=25
    X1_s50 = ndimage.gaussian_filter(I1, sigma=50).flatten().T.reshape(-1, 1) # T1 σ=50

    # --- T2 features ---
    I2 = plt.imread('../data/dataset_brains/1_1_t2.tif')
    X2 = I2.flatten().T.reshape(-1, 1)                          # T2 raw
    X2_s2 = ndimage.gaussian_filter(I2, sigma=2).flatten().T.reshape(-1, 1)   # T2 σ=2
    X2_s25 = ndimage.gaussian_filter(I2, sigma=25).flatten().T.reshape(-1, 1) # T2 σ=25
    X2_s50 = ndimage.gaussian_filter(I2, sigma=50).flatten().T.reshape(-1, 1) # T2 σ=50
    ...
    # cols: 0=T1, 1=T1s2, 2=T1s25, 3=T1s50, 4=T2, 5=T2s2, 6=T2s25, 7=T2s50
    X_data = np.concatenate((X1, X1_s2, X1_s25, X1_s50, X2, X2_s2, X2_s25, X2_s50), axis=1)
    features = ('T1', 'T1 σ=2', 'T1 σ=25', 'T1 σ=50', 'T2', 'T2 σ=2', 'T2 σ=25', 'T2 σ=50')

    if showFigs:
        # T2 features against each other (same as scatter_data_test but for T2)
        ax1 = util.scatter_data(X_data, Y, 4, 5)
        ax1.legend(); ax1.set_title(f'{features[4]} vs {features[5]}')

        ax2 = util.scatter_data(X_data, Y, 4, 6)
        ax2.legend(); ax2.set_title(f'{features[4]} vs {features[6]}')

        ax3 = util.scatter_data(X_data, Y, 4, 7)
        ax3.legend(); ax3.set_title(f'{features[4]} vs {features[7]}')

        # T1 vs T2 at same scales
        ax4 = util.scatter_data(X_data, Y, 0, 4)
        ax4.legend(); ax4.set_title(f'{features[0]} vs {features[4]}')

        ax5 = util.scatter_data(X_data, Y, 1, 5)
        ax5.legend(); ax5.set_title(f'{features[1]} vs {features[5]}')

        ax6 = util.scatter_data(X_data, Y, 2, 6)
        ax6.legend(); ax6.set_title(f'{features[2]} vs {features[6]}')

        ax7 = util.scatter_data(X_data, Y, 3, 7)
        ax7.legend(); ax7.set_title(f'{features[3]} vs {features[7]}')
...
def feature_stats_test():
    # first get the X dataset that has intensity + 4 gaussian values 
    X, Y = scatter_data_test(showFigs=False)
    I = plt.imread('../data/dataset_brains/1_1_t1.tif')
    # add the coordinate data to X dataset 
    c, coord_im = seg.extract_coordinate_feature(I)
    ...
    # features: T1, T1 σ=2, T1 σ=25, T1 σ=50, Coord
    features = ('T1', 'T1 σ=2', 'T1 σ=25', 'T1 σ=50', 'Coordinate')
    
    # compute mean and std per feature
    mean = np.mean(X_data, axis=0)
    st_dev = np.std(X_data, axis=0)
    
    # print 
    print(f'X_data            : {list(features)}')
    print(f'Mean              : {mean}')
...
def normalized_stats_test():
    ...
    st_dev = np.std(X_normalized, axis=0)
...
def distance_test():
    ...
    X, Y = seg.generate_gaussian_data(N=100, mu1=[0,0], mu2=[2,0], sigma1=[[1,0],[0,1]], sigma2=[[1,0],[0,1]])
    D = scipy.spatial.distance.cdist(X, X, metric='euclidean')
    plt.imshow(D)
    #------------------------------------------------------------------#


def small_samples_distance_test(X, Y):
    #------------------------------------------------------------------#
...
def minimum_distance_test(X, Y, C, D):
    ...
    ax.scatter(C[0,0], C[0,1], s=10, label="C0 (sample 1)", color='yellow')
    ax.scatter(C[1,0], C[1,1], s=10, label="C1 (sample 2)", color='green')
    ax.legend()
    
    min_index = np.argmin(D, axis=1)
    min_dist = D[np.arange(len(D)), min_index]
    
    counts = np.zeros(len(C))
    for i in range(len(C)):
        counts[i] = np.sum(min_index == i)
    ...
    print('How many samples in X are closest to each of the samples in C:')
    print(counts)
    print(counts[0], ' samples in X are closest to C0, ', counts[1],' samples in X are closest to C1' )
...
def distance_classification_test():
    ...
    # For each test point, find the closest training point
    min_index = np.argmin(D, axis=1)
    
    # Assign the label of the closest training point
    predicted_labels = train_labels[min_index].flatten()
    #------------------------------------------------------------------#
...
def kmeans_demo():
    ...
    w_vector = initial_w.reshape(clusters*M, 1).astype(float)
...
def kmeans_clustering_test():
    ...
    # Load data for subject 1, slice 1, brain/non-brain task
    X, Y, features = util.create_dataset(1, 1, 'brain')
    
    # Normalize the data
    X_norm, _ = seg.normalize_data(X)
    
    # Apply kmeans clustering
    predicted_labels = seg.kmeans_clustering(X_norm, K=2)
    
    # Reshape predicted labels back to image dimensions for visualization
    image_shape = (240, 240)
    predicted_image = predicted_labels.reshape(image_shape)
    true_image = Y.reshape(image_shape)
    
    # Visualize results side by side
    fig = plt.figure(figsize=(15, 5))
    ax1 = fig.add_subplot(131)
    ax1.imshow(plt.imread('../data/dataset_brains/1_1_t1.tif'))
    ax1.set_title('Original T1')
    ...
    ax3.imshow(predicted_image)
    ax3.set_title('KMeans Prediction')
...
def generate_train_test(N, task):
    ...
    if task == 'easy':
        ...
        mu1 = [0, 0]
        mu2 = [6, 0]                        # far apart
        sigma1 = [[0.5, 0], [0, 0.5]]       # tight clusters
        sigma2 = [[0.5, 0], [0, 0.5]]
        print("Easy dataset: μ1=", mu1, ", μ2=", mu2, ", σ1=", sigma1, ", σ2=",  sigma2)
        #-------------------------------------------------------------------#
    ...
    if task == 'hard':
        ...
        mu1 = [0, 0]
        mu2 = [1, 0]                        # close together
        sigma1 = [[2, 0], [0, 2]]           # spread out clusters
        sigma2 = [[2, 0], [0, 2]]
        print("Hard dataset: μ1=", mu1, ", μ2=", mu2, ", σ1=", sigma1, ", σ2=",  sigma2)
        #-------------------------------------------------------------------#
...
def easy_hard_data_classifier_test():
    ...
    predicted = seg.nn_classifier(trainX, trainY, testX)
    err = util.classification_error(testY, predicted)
    print('Error:\n{}'.format(err))
...
def knn_curve():
    ...
    for i in np.arange(len(k)):
        for j in np.arange(num_iter):
            ...
            subset_train_labels = train_labels[ix,:].ravel() # added ravel() to stop warnings 
