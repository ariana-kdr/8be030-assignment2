...
def segmentation_mymethod(train_data_matrix, train_labels_matrix, test_data, task='tissue'):
    ...
    n_pixels, n_features, n_subjects = train_data_matrix.shape

    train_data = train_data_matrix.reshape(n_pixels * n_subjects, n_features)
    train_labels = train_labels_matrix.reshape(n_pixels * n_subjects)

    train_data, test_data = seg.normalize_data(train_data, test_data)

    clf = RandomForestClassifier(n_estimators=100, random_state=0)
    # clf = make_pipeline(StandardScaler(), SVC(kernel='rbf', random_state=0, cache_size=500))
