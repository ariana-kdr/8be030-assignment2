...
def generate_gaussian_data(N=100, mu1=[0,0], mu2=[2,0], sigma1=[[1,0],[0,1]], sigma2=[[1,0],[0,1]]):
    ...
    # mu1 - 1x2 vector, mean of class 1    -- default = [0,0] -> centred on origin
    # mu2 - 1x2 vector, mean of class 2    -- default = [2,0] -> shifted 2 units along x axis 
    # sigma1 - 2x2 matrix, covariance of class 1
    # sigma2 - 2x2 matrix, covariance of class 2
    
    # Generate class 1
    # Rotate data according to covariance matrix (must be positive definite), and add the mean
        # return lower Cholesky decomposition of square matrix sigma1 (unpacks cov structure to apply to normal data)
    A = np.linalg.cholesky(sigma1)              
        # generate N samples of normal data in 2D
        # multiply by A to match covariance of sigma1
        # shift sample to centre at mu1 
    ...
    # Put the data together
        # so the dataset X has dimensions (2N, 2) 
        # where 2N is the number of samples
...
def cost_kmeans(X, w_vector):
    ...
    J = min_dist.dot(min_dist.T) / n # normalise cost by data rows 
...
def nn_classifier(train_data, train_labels, test_data):
    ...
    # Compute distances between every test point and every training point
    D = scipy.spatial.distance.cdist(test_data, train_data, metric='euclidean')
    
    # For each test point, find the index of the closest training point
    min_index = np.argmin(D, axis=1)
...
def knn_classifier(train_data, train_labels, test_data, k):
    ...
    predicted_labels = train_labels[sort_ix_k].astype(int) # cast to int 
    predicted_labels = scipy.stats.mode(predicted_labels, axis=1, keepdims=True)[0]
...
def segmentation_combined_knn(train_data_matrix, train_labels_matrix, test_data, k=1):
    ...
    # you probably can't return a bool anymore since we have 4 classes for tissue, so adjust this
    # you can check how it is implemented in project.segmentation_mymethod()
